...
#define BUFFER_SIZE 6000                    // up to 85% of dynamic memory (65,536 bytes)
#define SAMPLE_RATE  700000                  // see below maximum values
...
const int readPin2             = A16;
const int readPin3             = A17;
  ...
  if ((currentTime-lastInAvail) >= CHECKINPUT_INTERVAL) {
    ...
    if (Serial.available()) {
      ...
      // reading settings commands ------------
          ...
          if      (inNumber==1) { Vref = ADC_REFERENCE::REF_3V3; }
          ...
          else if (Vref == ADC_REFERENCE::REF_3V3) { Serial.println("1.2V"); }
          ...
          else if (inNumber==4)  { conv_speed = ADC_CONVERSION_SPEED::HIGH_SPEED; }
  ...
  //        else if (inNumber==11) { conv_speed = ADC_CONVERSION_SPEED::ADACK_4_0; }
    //      else if (inNumber==12) { conv_speed = ADC_CONVERSION_SPEED::ADACK_5_2; }
      //    else if (inNumber==13) { conv_speed = ADC_CONVERSION_SPEED::ADACK_6_2; }
          ...
          else if (conv_speed == ADC_CONVERSION_SPEED::HIGH_SPEED) { Serial.println("high speed 16bits");}
...
void setup_ADC_single(void) {
  ...
  memset((void*)buf_a, 0, sizeof(buf_a));
  // Initialize the ADC
 // if (sgain >1) { adc->adc0->enablePGA(sgain, ADC_0); }  else { adc->disablePGA(ADC_0); }         
  adc->adc0->setReference(Vref);
  adc->adc0->setAveraging(aver); 
  adc->adc0->setResolution(res); 
  if (((Vref == ADC_REFERENCE::REF_3V3) && (Vmax > 3.29))) { 
...
void start_ADC2(void) {
    ...
    a_full    = 0;
    b_full    = 0;
    adc->adc0->startSingleRead(readPin2);
    // frequency, hardware trigger and dma
    adc->adc0->startPDB(freq); // set ADC_SC2_ADTRG
    adc->adc0->enableDMA(); // set ADC_SC2_DMAEN
    dma0.enable();
...
void start_ADC3(void) {
    ...
    a_full    = 0;
    b_full    = 0;
    adc->adc0->startSingleRead(readPin3);
    // frequency, hardware trigger and dma
    adc->adc0->startPDB(freq); // set ADC_SC2_ADTRG
    adc->adc0->enableDMA(); // set ADC_SC2_DMAEN
    dma0.enable();
